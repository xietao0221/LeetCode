...
* if condition(3): (guess == target) return
...
(367, 274, 275) Perfect Square: different conditions
* while loop condition: while(left <= right)
* if condition(1): (mid * mid == target) return true;
* if condition(2): (mid * mid < target) left = mid + 1
* if condition(3): (mid * mid > target) right = mid - 1
...
(209, 363) use Kadane Algorithm => O(n)
...
1. Subarray, Substring(contiguous array), Subsequence(could delete some characters)
2. use hash map or hash set to save Point Class: (1) override equals() and hashcode() methods in Point Class (2) define MyPoint Class to composite Point Class and define equals() and hashcode() (3) inheritate Point using ‘class MyPoint extends Point {}’ and define equals() and hashcode()
3. TreeMap: lastKey() -> get the highest key value; lowerKey(val), get the greatest key strictly less than val; higherKey(val), get the smallest key strictly greater than val
4. LinkedList: poll()/peek() is the oldest one, pollLast()/peekLast() is the newest one
5. PriorityQueue: the default is min heap, queue.peek()/poll() is the smallest value; if use PriorityQueue as a sliding window, it always keep the k largest numbers
6. offer()/pollLast()/peekLast() [---> queue --->] poll()/peek()
7. offerLast()/peekLast() [---> deque --->] pollFirst()/peekFirst()
8. the comparison in array could use stack
...
package-private: class in same package
protected: subclass in other package or class in same package
public: all
